if(NCINE_BUILD_ANDROID)
	...
	set(ANDROID_PASSTHROUGH_ARGS -DNCINE_ROOT=${NCINE_ROOT} -DCMAKE_BUILD_TYPE=${CMAKE_BUILD_TYPE}
		-DNCINE_DYNAMIC_LIBRARY=${NCINE_DYNAMIC_LIBRARY} -DNCINE_STARTUP_TEST=${NCINE_STARTUP_TEST}
		-DEXTERNAL_ANDROID_DIR=${EXTERNAL_ANDROID_DIR} -DGENERATED_INCLUDE_DIR=${GENERATED_INCLUDE_DIR}
		-DNCINE_GRAPHICS_API=${NCINE_GRAPHICS_API} -DNCINE_WITH_OPENGL=${NCINE_WITH_OPENGL}
		-DNCINE_STRIP_BINARIES=${NCINE_STRIP_BINARIES} -DNCINE_WITH_PNG=${NCINE_WITH_PNG}
		-DNCINE_WITH_WEBP=${NCINE_WITH_WEBP} -DNCINE_WITH_QOI=${NCINE_WITH_QOI}
		-DQOI_SOURCE_DIR=${QOI_SOURCE_DIR} -DNCINE_WITH_AUDIO=${NCINE_WITH_AUDIO}
		-DNCINE_WITH_VORBIS=${NCINE_WITH_VORBIS} -DNCINE_WITH_OPENAL_EXT=${NCINE_WITH_OPENAL_EXT}
		-DNCINE_WITH_THREADS=${NCINE_WITH_THREADS} -DNCINE_WITH_JOBSYSTEM=${NCINE_WITH_JOBSYSTEM}
		-DNCINE_WITH_LUA=${NCINE_WITH_LUA} -DNCINE_WITH_SCRIPTING_API=${NCINE_WITH_SCRIPTING_API}
		-DNCINE_WITH_SCENEGRAPH=${NCINE_WITH_SCENEGRAPH} -DNCINE_WITH_ALLOCATORS=${NCINE_WITH_ALLOCATORS}
		...
		-DNCINE_WITH_CRASHPAD=${NCINE_WITH_CRASHPAD} -DCRASHPAD_BUILD_DIR=${CRASHPAD_ANDROID_DIR}
		-DCRASHPAD_SOURCE_DIR=${CRASHPAD_ANDROID_DIR})
	...
	if(TARGET ${NCINE_STARTUP_TEST})
		get_target_property(NCINE_STARTUP_TEST_EXTRA_SOURCES ${NCINE_STARTUP_TEST} SOURCES)
		# Filter out non-extra sources
		list(FILTER NCINE_STARTUP_TEST_EXTRA_SOURCES INCLUDE REGEX "\\.cpp$") # Keep only `.cpp` files
		list(FILTER NCINE_STARTUP_TEST_EXTRA_SOURCES EXCLUDE REGEX "${NCINE_STARTUP_TEST}\\.cpp$")

		list(LENGTH NCINE_STARTUP_TEST_EXTRA_SOURCES NUM_EXTRA_SOURCES)
		if(NUM_EXTRA_SOURCES GREATER 0)
			list(APPEND ANDROID_PASSTHROUGH_ARGS -DNCINE_STARTUP_TEST_EXTRA_SOURCES=${NCINE_STARTUP_TEST_EXTRA_SOURCES})
		endif()
	endif()
	...
	# Semicolons must be escaped, or `add_custom_command()` would re-splits them into separate `COMMAND` arguments
	string(REPLACE ";" "$<SEMICOLON>" NCINE_ANDROID_MODULE_PATH "${CMAKE_MODULE_PATH}")

	list(APPEND ANDROID_PASSTHROUGH_ARGS -DGENERATED_SOURCES="${GENERATED_SOURCES}" -DANDROID_GENERATED_FLAGS="${ANDROID_GENERATED_FLAGS}" -DCMAKE_MODULE_PATH="${NCINE_ANDROID_MODULE_PATH}")
	...
	set(GRADLE_COMPILESDK_VERSION 36)
	set(GRADLE_TARGETSDK_VERSION 36)
	...
	set(GRADLE_WITH_CRASHPAD "false")
	if(NCINE_WITH_CRASHPAD AND IS_DIRECTORY ${CRASHPAD_ANDROID_DIR})
		set(GRADLE_WITH_CRASHPAD "true")
	endif()
	...
	if(NOT NCINE_EMBED_SHADERS)
		foreach(SHADER_FILE ${NCINE_SHADER_FILE_LIST})
			list(APPEND SHADERS_ASSETS "${NCINE_DATA_DIR}/shaders/${SHADER_FILE}") # Add data directory prefix
		endforeach()
	endif()
	set(ANDROID_ASSETS ${FONT_TEXTURE_ASSETS_KTX} ${FONT_TEXTURE_ASSETS_WEBP} ${FONT_FNT_ASSETS}
		${SCRIPT_ASSETS} ${SOUND_ASSETS} ${TEXTURE_ASSETS_KTX} ${TEXTURE_ASSETS_WEBP} ${SHADERS_ASSETS})
	...
	if(NCINE_DYNAMIC_LIBRARY)
		...
		# To preserve directory structure the whole directory is copied instead of copying only the needed files from `${PRIVATE_HEADERS}`
		set(COPY_SRCINCLUDE_COMMAND ${CMAKE_COMMAND} -E copy_directory ${NCINE_ROOT}/src/include ${CMAKE_BINARY_DIR}/android/app/src/main/cpp/ncine/private_include/ncine)
	endif()
	if(NCINE_WITH_TRACY)
	...
	if(MSYS)
		set(MSYS2_DISABLE_PATH_CONV MSYS2_ARG_CONV_EXCL=-DGENERATED_SOURCES=)
	endif()
	...
	if(NCINE_WITH_CRASHPAD)
		foreach(ARCHITECTURE ${NCINE_NDK_ARCHITECTURES})
			find_program(ANDROID_CRASHPAD_HANDLER
				NAMES crashpad_handler
				PATH_SUFFIXES out/Android_${ARCHITECTURE} out/Default
				HINTS "${CRASHPAD_ANDROID_DIR}" "${CMAKE_PREFIX_PATH}"
				NO_CMAKE_FIND_ROOT_PATH # Skip sysroot on Android
			)
			if (EXISTS ${ANDROID_CRASHPAD_HANDLER})
				set(ANDROID_CRASHPAD_HANDLER_DEST_BINARY_DIR ${CMAKE_BINARY_DIR}/android/app/src/main/cpp/crashpad/lib/${ARCHITECTURE})
				add_custom_command(OUTPUT ${ANDROID_CRASHPAD_HANDLER_DEST_BINARY_DIR}/libcrashpad_handler.so
					COMMAND ${CMAKE_COMMAND} -E copy_if_different ${ANDROID_CRASHPAD_HANDLER} ${ANDROID_CRASHPAD_HANDLER_DEST_BINARY_DIR}/libcrashpad_handler.so
					COMMENT "Copying crashpad_handler for ${ARCHITECTURE}")
				add_custom_target(ncine_android_crashpad_handler_${ARCHITECTURE} DEPENDS ${ANDROID_CRASHPAD_HANDLER_DEST_BINARY_DIR}/libcrashpad_handler.so)
				set_target_properties(ncine_android_crashpad_handler_${ARCHITECTURE} PROPERTIES FOLDER "Android")
				add_dependencies(ncine_android ncine_android_crashpad_handler_${ARCHITECTURE})
			endif()
		endforeach()
	endif()
	...
	if(NCINE_INSTALL_DEV_SUPPORT)
		...
		if(NOT NCINE_DYNAMIC_LIBRARY)
			set(PRIVATE_HEADERS_ROOT ${NCINE_ROOT}/src/include)
			# Preserve folder structure when installing private headers
			foreach(PRIVATE_HEADER ${PRIVATE_HEADERS})
				file(RELATIVE_PATH RELATIVE_PATH ${PRIVATE_HEADERS_ROOT} ${PRIVATE_HEADER})
				get_filename_component(RELATIVE_DIR ${RELATIVE_PATH} DIRECTORY)

				install(FILES ${PRIVATE_HEADER} DESTINATION ${ANDROID_INSTALL_DESTINATION}/app/src/main/cpp/ncine/include/ncine/${RELATIVE_DIR} COMPONENT android)
			endforeach()
		endif()
		if(NCINE_WITH_TRACY)
