...
if(IS_DIRECTORY ${DEBUGINFO_DIR} AND ${NCINE_DEBUGINFO} STREQUAL "INSTALL")
	set(CPACK_COMPONENTS_ALL "${CPACK_COMPONENTS_ALL};symbols")
	if(NCINE_BUILD_TESTS)
		set(CPACK_COMPONENT_SYMBOLS_DEPENDS libraries tests)
	else()
		set(CPACK_COMPONENT_SYMBOLS_DEPENDS libraries)
	endif()
	set(CPACK_COMPONENT_SYMBOLS_DISPLAY_NAME "Symbols")
	set(CPACK_COMPONENT_SYMBOLD_DESCRIPTION "Debug information files")
endif()
...
if(MSVC OR INSTALL_LIBRARIES_MINGW)
	if(GLEW_FOUND AND NCINE_WITH_ANGLE)
	...
	if(SDL3_FOUND AND NCINE_WITH_SDL3)
		...
		install(FILES ${SDL3_IMPORTED_LOCATION} DESTINATION ${RUNTIME_INSTALL_DESTINATION} COMPONENT libraries)
	endif()
	...
	if(INSTALL_LIBRARIES_MINGW)
		install(FILES ${MINGW_SYSTEMLIB_LOCATIONS} DESTINATION ${RUNTIME_INSTALL_DESTINATION} COMPONENT libraries)
	endif()
elseif(LINUX AND NOT CMAKE_SKIP_RPATH AND NOT CMAKE_SKIP_INSTALL_RPATH)
	function(install_linux_library TARGET_NAME)
		...
		install(PROGRAMS ${LIB_LOCATION} DESTINATION ${LIBRARY_INSTALL_DESTINATION} COMPONENT libraries) # using `PROGRAMS` to add executable flag
		...
		while(LIB_REGEX_MATCH)
			...
			install(CODE "file(CREATE_LINK \"${LIB_NAME}\" \"\${CMAKE_INSTALL_PREFIX}/${LIBRARY_INSTALL_DESTINATION}/${LIB_NAME_STRIPPED}\" SYMBOLIC)")
			string(REGEX MATCH "\\.[0-9]+$" LIB_REGEX_MATCH ${LIB_NAME_STRIPPED})
		endwhile()
	endfunction()

	set_target_properties(ncine PROPERTIES INSTALL_RPATH "$ORIGIN;$ORIGIN/../${LIBRARY_INSTALL_DESTINATION}")
	# Setting also the `BUILD_RPATH` property for building projects that use a build directory for their `nCine_DIR`
	set_target_properties(ncine PROPERTIES BUILD_RPATH "$ORIGIN;$ORIGIN/../${LIBRARY_INSTALL_DESTINATION}")
	set_target_properties(ncine PROPERTIES BUILD_RPATH_USE_ORIGIN TRUE)
	...
	if(SDL3_FOUND AND NCINE_WITH_SDL3)
		install_linux_library(SDL3::SDL3)
	endif()
	if(SDL2_FOUND AND NCINE_WITH_SDL2)
	...
	if(WEBP_FOUND AND NCINE_WITH_WEBP)
		...
		if(TARGET WebP::SharpYUV)
			install_linux_library(WebP::SharpYUV)
		endif()
	endif()
	if(OPENAL_FOUND AND NCINE_WITH_AUDIO)
...
elseif(APPLE)
	...
	if(SDL3_FOUND AND NCINE_WITH_SDL3)
		install(DIRECTORY ${CMAKE_FRAMEWORK_PATH}/sdl3.framework DESTINATION ${FRAMEWORKS_INSTALL_DESTINATION} COMPONENT libraries)
	endif()
	if(SDL2_FOUND AND NCINE_WITH_SDL2)
...
if(NCINE_INSTALL_DEV_SUPPORT)
	...
	if(NOT NCINE_DYNAMIC_LIBRARY)
		...
		foreach(PRIVATE_HEADER ${PRIVATE_HEADERS})
			...
			install(FILES ${PRIVATE_HEADER} DESTINATION ${INCLUDE_INSTALL_DESTINATION}/ncine/${RELATIVE_DIR} COMPONENT devsupport)
		endforeach()
	endif()
	if(NCINE_WITH_TRACY)
	...
	install(FILES .clang-format .editorconfig .gitignore DESTINATION ${GENERIC_INSTALL_DESTINATION}/project COMPONENT devsupport)
	install(FILES cmake/check_atomic.cmake cmake/FindGLFW.cmake cmake/FindOpenGLES2.cmake cmake/FindSDL2.cmake cmake/FindSDL3.cmake cmake/FindVorbis.cmake cmake/FindWebP.cmake DESTINATION ${GENERIC_INSTALL_DESTINATION}/project/cmake COMPONENT devsupport)
	if(WIN32)
...
if(NOT NCINE_EMBED_SHADERS)
	foreach(SHADER_FILE ${NCINE_SHADER_FILE_LIST})
		install(FILES ${NCINE_SRC_SHADERS_DIR}/${SHADER_FILE} DESTINATION ${SHADERS_INSTALL_DESTINATION} COMPONENT libraries)
	endforeach()
endif()
...
if(NCINE_WITH_CRASHPAD)
	if(EXISTS ${CRASHPAD_HANDLER_EXE})
		...
		# so to avoid installing the crashpad handler system wide
		if(NOT LINUX OR (LINUX AND NOT DEFINED NCINE_OPTIONS_PRESETS))
			# The Crashpad handler is installed alongside the "libraries" component to be available for other projects
			install(FILES ${CRASHPAD_HANDLER_EXE} DESTINATION ${RUNTIME_INSTALL_DESTINATION} COMPONENT libraries)
		endif()
	endif()
endif()
if(IS_DIRECTORY ${DEBUGINFO_DIR} AND ${NCINE_DEBUGINFO} STREQUAL "INSTALL")
