...
struct alignas(64) Job
{
	...
	static_assert(UNFINISHED_BITS + CONTINUATION_BITS + FLAGS_BITS == 32);

	static constexpr uint32_t UNFINISHED_MASK = (1u << UNFINISHED_BITS) - 1u;
	static constexpr uint32_t CONTINUATION_MASK = ((1u << CONTINUATION_BITS) - 1u) << 16;
	static constexpr uint32_t FLAGS_MASK = 0xFu << 28;

	static constexpr uint32_t CONTINUATION_SHIFT = sizeof(uint32_t) * 8 - UNFINISHED_BITS;
	...
	/// Layout: [ flags:4 | continuation count:12 | unfinished jobs:16 ]
...
static_assert(JobNumContinuations <= (1u << Job::CONTINUATION_BITS));
