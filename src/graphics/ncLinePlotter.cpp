...
ncLineVariable::ncLineVariable(unsigned int numValues, float rejectDelay)
...
unsigned int ncLinePlotter::addVariable(unsigned int numValues, float rejectDelay)
{
	ncLineVariable* variable = new ncLineVariable(numValues, rejectDelay);
...
void ncLinePlotter::draw(ncRenderQueue& renderQueue)
{
