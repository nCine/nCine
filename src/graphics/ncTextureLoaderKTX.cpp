...
uint8_t ncTextureLoaderKTX::fileIdentifier_[12] =
...
ncTextureLoaderKTX::ncTextureLoaderKTX(const char *filename)
	: ncITextureLoader(filename)
{
	init();
}

ncTextureLoaderKTX::ncTextureLoaderKTX(ncIFile *fileHandle)
...
void ncTextureLoaderKTX::readHeader(KtxHeader &header)
{
	...
	if (checkPassed)
	{
		...
		headerSize_ = 64 + ncIFile::int32FromLE(header.bytesOfKeyValueData);
		width_ = ncIFile::int32FromLE(header.pixelWidth);
		height_ = ncIFile::int32FromLE(header.pixelHeight);
...
void ncTextureLoaderKTX::parseFormat(const KtxHeader& header)
{
	const ncGfxCapabilities& gfxCaps = ncServiceLocator::gfxCapabilities();

	GLenum internalFormat = ncIFile::int32FromLE(header.glInternalFormat);
	GLenum type = ncIFile::int32FromLE(header.glType);
