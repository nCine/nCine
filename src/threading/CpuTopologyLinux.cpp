...
void fillCpuInfos(nctl::Array<LinuxCpuInfo> &linuxCpuInfos, nctl::Array<CpuTopology::CpuInfo> &cpuInfos)
{
	...
	for (unsigned int i = 0; i < linuxCpuInfos.size(); i++)
	{
		...
		LOGD_X("CPU #%02d (tier %u) - core: %d, package: %d, capacity: %d, max freq: %d Mhz, logical: %d",
		       linuxInfo.id, info.tier, linuxInfo.coreId, linuxInfo.packageId, linuxInfo.capacity, linuxInfo.maxFreqKhz / 1024, linuxInfo.isLogical);
	}
}
