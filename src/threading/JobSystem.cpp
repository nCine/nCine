    ...
    : IJobSystem(numThreads), commonData_(jobPool_, queueSem_)
{
	...
	// Note: not setting the affinity mask of the main thread, leaving the fastest core without pinned threads
}
...
void JobSystem::wait(JobId jobId)
{
	...
	Job *job = jobPool_.retrieveJob(jobId);
	// Note: Not asserting on invalid jobs because small ones can finish before reaching `wait()`
	//ASSERT(job != nullptr);
	if (job == nullptr)
		return;
