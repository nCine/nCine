...
// TODO: Instead of splitting the header, just write it at address zero of the buffer and lose one byte
// TODO: Test log entries correctness with a lot of `emptyJobWithStatistics`
...
bool LogEntryQueue::enqueue(const char *entry, unsigned int length)
{
	...
	uint32_t nextWritePos = currentWritePos + LengthHeaderSize + length;
	if (nextWritePos > BufferLength)
		nextWritePos -= BufferLength;
	...
	// Encode the length as two bytes
	const LengthHeaderType lengthHeader = static_cast<LengthHeaderType>(length);
	const uint32_t headerStart = currentWritePos;
	entriesBuffer_[headerStart % BufferLength] = static_cast<uint8_t>(lengthHeader & 0xFF);
	entriesBuffer_[(headerStart + 1) % BufferLength] = static_cast<uint8_t>((lengthHeader >> 8) & 0xFF);

	// Write the entry data with wrap-aware memcpy
	const uint32_t entryStart = (currentWritePos + LengthHeaderSize) % BufferLength;
