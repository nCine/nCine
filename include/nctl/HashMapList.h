  ...
  private:
	...
	class HashBucket
	{
	  public:
		...
		template <class... Args> Node *findOrConstructNode(hash_t hash, const K &key, Args &&...args);
...
template <class K, class T, class HashFunc>
bool HashMapList<K, T, HashFunc>::HashBucket::contains(hash_t hash, const K &key) const
{
	const Node *node = findNode(hash, key);
	return (node != nullptr);
}
...
template <class K, class T, class HashFunc>
template <class... Args>
typename HashMapList<K, T, HashFunc>::Node *HashMapList<K, T, HashFunc>::HashBucket::findOrConstructNode(hash_t hash, const K &key, Args &&...args)
{
	if (size_ == 0)
	{
...
bool HashMapList<K, T, HashFunc>::HashBucket::insert(hash_t hash, const K &key, const T &value)
{
	...
	findOrConstructNode(hash, key, value);
	return true;
}
...
bool HashMapList<K, T, HashFunc>::HashBucket::insert(hash_t hash, const K &key, T &&value)
{
	...
	findOrConstructNode(hash, key, nctl::move(value));
	return true;
}
...
bool HashMapList<K, T, HashFunc>::HashBucket::emplace(hash_t hash, const K &key, Args &&...args)
{
	...
	findOrConstructNode(hash, key, nctl::forward<Args>(args)...);
	return true;
}
...
T &HashMapList<K, T, HashFunc>::operator[](const K &key)
{
	...
	HashBucket &bucket = retrieveBucket(hash);
	// Storing previous bucket size to infer if a new node was created or not
	const unsigned int oldBucketSize = bucket.size_;
	T &node = bucket.findOrConstructNode(hash, key)->value;
	if (bucket.size_ > oldBucketSize)
		size_++;
...
{
	const hash_t hash = hashFunc_(key);
	const bool inserted = retrieveBucket(hash).insert(hash, key, value);
	size_ += inserted ? 1 : 0;
	return inserted;
}
...
{
	const hash_t hash = hashFunc_(key);
	const bool inserted = retrieveBucket(hash).insert(hash, key, nctl::move(value));
	size_ += inserted ? 1 : 0;
	return inserted;
}
...
/*! \return True if the element has been inserted */
template <class K, class T, class HashFunc>
bool HashMapList<K, T, HashFunc>::insert(const Pair<K, T> &pair)
{
	const hash_t hash = hashFunc_(pair.first);
...
{
	const hash_t hash = hashFunc_(key);
	const bool emplaced = retrieveBucket(hash).emplace(hash, key, nctl::forward<Args>(args)...);
	size_ += emplaced ? 1 : 0;
	return emplaced;
}
...
template <class K, class T, class HashFunc>
bool HashMapList<K, T, HashFunc>::contains(const K &key) const
{
	const hash_t hash = hashFunc_(key);
