		...
		if (ImGui::Begin("apptest_jobsystem", &showImGui))
		{
			...
			if (ImGui::Button("Enqueue jobs"))
			{
				...
				for (int rep = 0; rep < numRepetitions; rep++)
				{
					...
					submitStats.addValue(totalTimestamp.millisecondsSince());

					{
						ZoneScopedN("Job Wait");
						partialTimestamp = nc::TimeStamp::now();
						jobSystem.wait(rootJobId);
						waitStats.addValue(partialTimestamp.millisecondsSince());
					}

					totalStats.addValue(totalTimestamp.millisecondsSince());
				}
				calculateStatistics();
