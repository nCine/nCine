...
void MyEventHandler::onFrameStart()
{
	if (!pause_)
	{
		...
		dummy_->setPositionX(nc::theApplication().width() * 0.5f + sine * 100.0f);
		dummy_->setPositionY(nc::theApplication().height() * 0.5f + cosine * 150.0f);
		...
		nc::theApplication().rootNode().setPositionX((sine + 1.0f) * 50.0f);
		nc::theApplication().rootNode().setPositionY((cosine + 1.0f) * 50.0f);
