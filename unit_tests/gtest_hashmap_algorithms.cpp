...
TEST_F(HashMapAlgorithmsTest, FillNElements)
{
	...
	nctl::HashMap<int, int, nctl::FixedHashFunc<int> >::ConstIterator halfIt = nctl::next(hashmap_.begin(), Size / 2);

	int n = 0;
	for (nctl::HashMap<int, int, nctl::FixedHashFunc<int> >::ConstIterator i = hashmap_.begin(); i != halfIt; ++i)
	{
		ASSERT_EQ(i.key(), n);
		ASSERT_EQ(*i, 0);
		n++;
	}
	for (nctl::HashMap<int, int, nctl::FixedHashFunc<int> >::ConstIterator i = halfIt; i != hashmap_.end(); ++i)
	{
		ASSERT_EQ(i.key(), n);
...
TEST_F(HashMapAlgorithmsTest, GenerateN)
{
	...
	nctl::HashMap<int, int, nctl::FixedHashFunc<int> >::ConstIterator numElementsIt = nctl::next(hashmap_.begin(), numElements);
	int n = 0;
	for (nctl::HashMap<int, int, nctl::FixedHashFunc<int> >::ConstIterator i = hashmap_.begin(); i != numElementsIt; ++i)
	{
		ASSERT_EQ(i.key(), n);
		ASSERT_EQ(*i, n + value);
		n++;
	}
	for (nctl::HashMap<int, int, nctl::FixedHashFunc<int> >::ConstIterator i = numElementsIt; i != hashmap_.end(); ++i)
	{
		ASSERT_EQ(i.key(), n);
